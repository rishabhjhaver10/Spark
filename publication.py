...
conf = SparkConf().setMaster("local").setAppName("publication")
sc = SparkContext(conf = conf)

data = sc.textFile("file:///C:/Users/risha/Downloads/hw3data/c0001")
data_1 = data.map(lambda x : x.split(':::'))
data_2 = data_1.map(lambda x : (x[1], x[2]))
data_3 = data_2.flatMap(lambda (x, y) : zip(x.split('::'), itertools.repeat(y)))
remove_punct = lambda x : x not in string.punctuation
data_4 = data_3.map(lambda (x, y) : (x, filter(remove_punct, y))).map(lambda (x, y) : (x, y.lower()))
data_5 = data_4.flatMap(lambda (x, y) : zip(itertools.repeat(x), y.split()))
data_6 = data_5.groupByKey().mapValues(list).sortByKey()
...
def unique_dict(item):
    word_dict = {}
    for word in item:
        if ((word not in all_stop_words.keys()) and (len(word) > 1)):
...
data_7 = data_6.map(lambda (x, y) : (x, unique_dict(y)))    
data_8 = data_7.take(100)
